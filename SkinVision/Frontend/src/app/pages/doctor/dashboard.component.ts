        ...
        <div class="sidebar">
          <div class="section">
            ...
            <div class="quick-actions">
              <button class="action-btn" (click)="navigateTo('/doctor/cases')">
                <span class="action-icon">📁</span>
                <span class="action-text">View All Cases</span>
              </button>
              <button class="action-btn" (click)="toggleAvailability()">
                <span class="action-icon">{{ isAvailable ? '⏸️' : '▶️' }}</span>
                <span class="action-text">{{ isAvailable ? 'Go Offline' : 'Go Online' }}</span>
              </button>
              <button class="action-btn" (click)="reviewNextCase()">
                <span class="action-icon">▶️</span>
                <span class="action-text">Review Next Case</span>
              </button>
            </div>
          </div>
...
export class DoctorDashboardComponent {
  ...
  toggleAvailability() {
    ...
    alert(this.isAvailable ? 'You are now online and accepting cases' : 'You are now offline');
  }

  reviewNextCase() {
    if (this.assignedCases.length > 0) {
      this.router.navigate(['/doctor/case', this.assignedCases[0].id]);
    } else {
      alert('No cases in your queue');
    }
  }
}
